...
// 导入 Counter 程序的 TypeScript 类型定义
...
async function main() {
  ...
  // ==================== 8. 计算 PDA（程序派生地址） ====================
  // 计算 PDA - 使用种子（"counter" + 用户公钥）派生唯一的账户地址
  // PDA 确保每个用户都有独立的 counter 账户
  const [counterPda] = PublicKey.findProgramAddressSync(
    [Buffer.from("counter"), payer.publicKey.toBuffer()],    // 种子：固定字符串 + 用户公钥
    program.programId                                        // 程序 ID
  );

  // ==================== 9. 构建并发送交易 ====================
  // 构建 initialize 指令 - 使用 accountsPartial 避免类型检查问题
  // 调用程序的 initialize 方法，初始化计数器
  const tx = await program.methods
    .initialize()
    .accountsPartial({                  // 指定交易所需的账户（部分模式，更灵活）
      user: payer.publicKey,            // 用户账户（签名者和支付者）
      counter: counterPda,              // PDA 账户（存储数据）
  ...
  console.log("交易日志(initialize):", txInfo?.meta?.logMessages);
  
  // 调用程序的increment方法，将PDA账户中的 count 加 1
  ...
  console.log("Transaction Signature(increment)", tx2);

  const txInfo2 = await connection.getParsedTransaction(tx2);
  console.log("交易日志(increment):", txInfo2?.meta?.logMessages);


  // ==================== 10. 获取单个 PDA 账户信息 ====================
  // 获取某个PDA counter 账户信息（使用 Anchor 的类型安全解析）
  ...
  // 可选：批量获取多个账户的信息
  // const accounts = await connection.getMultipleAccountsInfo([counterPda, payer.publicKey, program.programId]);
    ...
    try {
      ...
      if (Buffer.isBuffer(account.account.data)) {
        ...
        if (account.account.data.length >= 8) {
          // "counter" 账户类型 对应 IDL 中的 Counter 结构体
          // 使用 Anchor 的 coder 手动解码账户数据
          const decodedData = program.coder.accounts.decode("counter", account.account.data);
          console.log("📊 解析的账户数据:");

          console.log(`  Count: ${decodedData.count.toString()}`);
        } else {
          console.log("⚠️  账户数据太短，跳过");
        }
      }
    } catch (error: any) {
      // 只在调试时显示详细错误，生产环境可以静默跳过非 counter 账户
      if (error.message.includes("Invalid account discriminator")) {
        console.log("⚠️  跳过非 counter 类型账户");
      } else {
