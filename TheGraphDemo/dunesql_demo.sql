...
-- 不同 JOIN 的结果
-- JOIN 类型	      说明	      返回结果
-- INNER JOIN	      默认的JOIN	只返回两个表都匹配的行
-- LEFT JOIN	      左连接	    返回左表所有行 + 右表匹配的行（不匹配显示 NULL）
-- RIGHT JOIN	      右连接	    返回右表所有行 + 左表匹配的行（不匹配显示 NULL）
-- FULL OUTER JOIN	全连接	    返回所有行（匹配的关联，不匹配的显示 NULL）
...
-- 连表作为中间表查询过去一周的以太坊每天的交易量(单位：美元)
-- 步骤 1：定义 CTE 临时表(Common Table Expression，公共表表达式)
...
-- 使用 prices.usd 表查询过去30天每天的ETH 的最高 最低 平均价格  
-- ETH 价格 (30D) 
SELECT
...
/* 使用 prices.usd 表查询 ETH 过去7天 每天的 以美元计价的交易量 */
-- 步骤 1：定义 CTE 临时表
with txv as 
