...
contract LaunchPadV2 is Ownable {
    ...
    struct SaleInfo {
        ...
        IERC20Metadata saleToken;         // Token being sold
    ...
    function createTokenAndSale(
        ...
        sales.push(SaleInfo({
            creator: msg.sender,
            saleToken: IERC20Metadata(tokenAddress),
            paymentToken: IERC20(_paymentToken),
            price: _price,
    ...
    function buy(uint256 _saleId, uint256 _amount) external {
        ...
        uint256 cost = (_amount * sale.price) / (10 ** uint256(sale.saleToken.decimals()));
    ...
    function finalize(uint256 _saleId) external {
        ...
        uint256 totalRaised = (sale.soldAmount * sale.price) / (10 ** uint256(sale.saleToken.decimals()));
