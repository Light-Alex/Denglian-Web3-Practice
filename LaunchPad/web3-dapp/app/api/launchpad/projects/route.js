...
async function fetchChainProjects() {
  ...
  try {
    ...
    for (let i = 0; i < Number(saleCount); i++) {
      ...
      const decimal = BigInt(10) ** BigInt(decimals)
      const priceInEther = Number(price) / 1e18

      // Calculate raised amount (soldAmount * price / 1e18)
      const raised = (Number(soldAmount) / Number(decimal)) * priceInEther
      const goal = (Number(totalAmount) / Number(decimal)) * priceInEther
      ...
      projects.push({
        ...
        totalSupply: (Number(totalAmount) / Number(decimal)).toLocaleString('en-US', { maximumFractionDigits: 0 }),
        startTime: Number(startTime) * 1000, // Convert to ms
        endTime: Number(endTime) * 1000,
        minPurchase: (Number(minPurchase) / Number(decimal)).toString(),
        maxPurchase: (Number(maxPurchase) / Number(decimal)).toString(),
        status,
