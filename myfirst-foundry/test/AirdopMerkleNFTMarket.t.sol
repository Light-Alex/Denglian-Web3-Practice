...
contract AirdopMerkleNFTMarketTest is Test {
    ...
    address public whitelistedBuyer = vm.addr(3);
    address public nonWhitelistedBuyer = vm.addr(4);
    ...
    function setUp() public {
        ...
        console.log("leaves[0]:", vm.toString(leaves[0]));

        leaves[1] = keccak256(abi.encodePacked(buyer));
        console.log("leaves[1]:", vm.toString(leaves[1]));

        merkleRoot = _generateMerkleRoot(leaves);
        whitelistProof = _generateMerkleProof(leaves, 0);
        console.log("Merkle Root:", vm.toString(merkleRoot));
        console.log("Whitelist Proof 0:", vm.toString(whitelistProof[0]));
    ...
    // 简化版 Merkle Proof：仅生成 1 层证明，适用于二叉树的测试场景
    ...
    function testMulticallPermitAndClaim() public {
        ...
        (uint8 v, bytes32 r, bytes32 s) = signERC20Permit(
            3,
            whitelistedBuyer,
            address(market),
            discountedPrice,
