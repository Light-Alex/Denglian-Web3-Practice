...
contract DAOTest is Test {
    ...
    address public deployer = address(0x1);  // 部署合约的地址
    address public user1 = address(0x2);     // 测试用户1
    address public user2 = address(0x3);     // 测试用户2
    address public user3 = address(0x4);     // 测试用户3
    address public recipient = address(0x5); // 接收提案执行结果的地址

    uint256 public constant INITIAL_SUPPLY = 100000 * 10**18; // 初始DAO Token供应量(用于计算投票权重)
    ...
    function setUp() public {
        ...
        // Deploy BankDAO (deployer as initial admin)
        bank = new BankDAO(deployer);

        // Deploy Gov
        gov = new Gov(address(voteToken), address(bank));

        // Set Gov as BankDAO admin
        ...
        console.log("BankDAO address:", address(bank));
        console.log("Gov address:", address(gov));
        console.log("BankDAO balance:", bank.getBalance());
    ...
    function _testExecution() internal {
        ...
        console.log("BankDAO balance before execution:", bankBalanceBefore);
        ...
        console.log("BankDAO balance after execution:", bankBalanceAfter);
