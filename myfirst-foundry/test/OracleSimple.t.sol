...
contract MockUniswapV2Factory is IUniswapV2Factory {
    ...
    function setFeeTo(address fee) external override {
        return;
    }

    function setFeeToSetter(address feeSetter) external override {
        return;
    }
}
...
contract MockUniswapV2Pair is IUniswapV2Pair {
    ...
    function setReserves(uint112 _reserve0, uint112 _reserve1, uint32 _blockTimestampLast) external {
        ...
        // 在更新储备之前，使用当前储备计算累积价格
        if (previousTimestamp > 0 && reserve0 > 0 && reserve1 > 0) {
            uint32 timeElapsed = _blockTimestampLast - previousTimestamp;
            if (timeElapsed > 0) {
                // 使用当前（旧）储备来计算这段时间的价格累积
...
contract OracleSimpleTest is Test {
    ...
    function testPriceUpdateAndConsult() public {
        ...
        console.log("ETH amount for 1 Meme (ETH/Meme):", expected);
        ...
        console.log("Meme amount for 1 ETH (Meme/ETH):", expected);
    ...
    function testMultipleUpdates() public {
        ...
        for (uint i = 1; i <= 3; i++) {
            ...
            console.log("Meme price (ETH/Meme):", memePrice);
            console.log("ETH price (Meme/ETH):", ethPrice);
    ...
    function testPriceCalculation() public {
        ...
        console.log("Actual ETH amount for 1 Meme (ETH/Meme):", actualEthAmount);
        ...
        console.log("Actual Meme amount for 1 ETH (Meme/ETH):", actualMemeAmount);
    ...
    function testPriceVolatility() public {
        ...
        console.log("TWAP Meme price after volatility (1 Meme in ETH) (ETH/Meme):", memePrice);
    ...
    function addLiquidityForTesting() internal {
        ...
        // 需要根据代币顺序正确设置储备
        if (memeToken == token0) {
            // memeToken 是 token0，WETH 是 token1
            MockUniswapV2Pair(pair).setReserves(
                uint112(1000 * 10**18),  // reserve0 = 1000 Meme
                uint112(0.1 ether),       // reserve1 = 0.1 ETH
                uint32(block.timestamp)
            );
        } else {
            // memeToken 是 token1，WETH 是 token0
            MockUniswapV2Pair(pair).setReserves(
                uint112(0.1 ether),       // reserve0 = 0.1 ETH
                uint112(1000 * 10**18),  // reserve1 = 1000 Meme
                uint32(block.timestamp)
            );
        }
    }
