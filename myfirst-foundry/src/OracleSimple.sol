...
library FixedPoint {
    // 带有 112 位小数位的无符号定点数
    // uq112x112: 112位整数 + 112位小数
    struct uq112x112 {
        uint224 _x; // // 总共224位
    }

    // 带有 224 位小数位的无符号定点数
    // uq144x112: 144位整数 + 112位小数（用于乘法后的结果）
    struct uq144x112 {
        uint256 _x; // 总共256位
    }

    uint8 private constant RESOLUTION = 112; // 小数位数
    uint private constant Q112 = 2**112;     // 缩放因子
    uint private constant Q224 = 2**224;     // 双精度缩放因子

    // 将 uint112 编码为 uq112x112
    // 将普通整数转换为定点数格式
    // 例如: encode(150.5) = 150.5 * 2**112 = 150 * 2**112 + 0.5 * 2**112 = 150 * 2 * 2 ** 111 + 2 ** 111 = (150 * 2 + 1) * 2 ** 111 = 301 * 2 ** 111
    ...
    // 编码大整数
    // 例如: encodeUint(123456) = 123456 * 2**112 = 28242953648123456
    ...
    // 将定点数转换回普通整数（丢失小数部分）
    // 例如: 输入150.5 的定点数表示, 输出150(小数部分被截断)
    ...
    // 将乘法结果转换回普通整数（丢失小数部分）
    // 例如: 输入150.5 * 100 的乘积, 输出15050 (整数部分)
...
contract OracleSimple {
    ...
    function update() external {
        ...
        // 计算平均价格
        // 溢出是预期的，强制转换永远不会截断
        // Solidity 会自动为这个 struct 生成一个"构造函数"，允许你直接传入字段值来创建实例
    ...
    function getMemePrice(uint amountIn) external view returns (uint) {
        if (memeToken == token0) {
            // memeToken 是 token0，price0 = token1/token0 = ETH/Meme
            return consult(token0, amountIn);
        } else {
            // memeToken 是 token1，price1 = token0/token1 = ETH/Meme
            // consult(token1, amountIn) 返回 amountIn × price1
