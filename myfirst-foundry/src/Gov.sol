...
contract Gov is ReentrancyGuard {
    ...
    uint256 public votingPeriod = 100;     // 投票区间（区块数）
    uint256 public proposalThreshold = 1000 * 10**18;  // 提案门槛
    uint256 public quorum = 4000 * 10**18;  // 赞成票门槛
    ...
    function castVote(uint256 proposalId, bool support) external {
        ...
        uint256 weight = voteToken.getPastVotes(msg.sender, proposal.startBlock); // 获取该用户在startBlock区块的投票权重
     ...
     * @param proposalId 提案ID
     */
    function getProposalBlockRange(uint256 proposalId) external view returns (
        uint256 startBlock,
        uint256 endBlock
    ) {
        require(proposalId <= proposalCount && proposalId > 0, "Gov: invalid proposal id");
