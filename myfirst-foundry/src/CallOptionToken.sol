...
contract CallOptionToken {
    ...
    uint256 public totalSupply; // 已发行的期权Token总数量
    ...
    uint256 public strikePrice;         // 行权价格 (以paymentToken计价) --- 即约定好的购买标的资产的价格，在过期前均可以行权
    uint256 public expirationDate;      // 到期日期
    uint256 public underlyingAmount;    // 每个期权Token对应的标的资产数量
    bool public expired;                // 期权是否已过期
    
    uint256 public totalUnderlyingDeposited; // 总共存入的标的资产数量
    ...
    // 期权事件
    event OptionIssued(address indexed issuer, uint256 underlyingAmount, uint256 optionTokens); // 发行期权
    event OptionExercised(address indexed exerciser, uint256 optionTokens, uint256 underlyingReceived, uint256 paymentPaid); // 行权期权
    event OptionExpired(address indexed issuer, uint256 remainingUnderlying); // 过期销毁期权
    event TradingPairCreated(address indexed token0, address indexed token1, uint256 liquidity); // 创建交易对
    ...
    constructor(
        ...
        address _underlyingAsset, // 标的资产地址 (ETH使用WETH)
        address _paymentToken, // 支付代币 (USDT)
        uint256 _strikePrice, // 行权价格
        uint256 _expirationDate, // 到期日期
        uint256 _underlyingAmount // 每个期权Token对应的标的资产数量
    ) {
        name = _name;
        symbol = _symbol;
        decimals = 18;
        ...
        underlyingAmount = _underlyingAmount; // 例如：1个期权Token = 0.001 ETH, 表示一个期权token可交易的标的资产数量
        expired = false;
    }
    ...
    // 发行期权Token（项目方角色）
    // _underlyingAmountDeposit: 项目方存入的标的资产数量
