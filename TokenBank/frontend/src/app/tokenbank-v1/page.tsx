'use client'; // 告诉 Next.js 这个文件是一个客户端组件

// 引入 React(前端框架, 用于构建用户界面) 相关 hooks
// useState: 状态管理
// useEffect: 副作用管理, 3种使用模式: 每次渲染后执行、仅在挂载时执行、依赖项变化时执行
import { useState, useEffect } from 'react';

// 引入 wagmi(一个用于以太坊和EVM兼容链的React Hooks库, We're All Gonna Make It) 相关的客户端hooks
// useAccount: 获取账户信息
// useReadContract: 读取合约数据
// useWriteContract: 写入合约交易
// useWaitForTransactionReceipt: 等待交易确认
import { useAccount, useReadContract, useWriteContract, useWaitForTransactionReceipt } from 'wagmi';

// viem: 一个用于以太坊的 TypeScript 接口库，提供了与以太坊区块链交互的低级工具。它的名字来源于拉丁语 "viem"，意思是"方式"或"路径"
// parseEther: 将以太币金额转换为 wei 单位的 BigInt
// formatEther: 将 wei 单位的 BigInt 转换为以太币金额的字符串表示
...
export default function TokenBankV1() {
  ...
  // depositAmount: 当前输入的存款金额（字符串类型）, 初始值为空字符串
  // setDepositAmount: 更新存款金额的函数
  const [depositAmount, setDepositAmount] = useState('');

  // withdrawAmount: 当前输入的取款金额（字符串类型）, 初始值为空字符串
  // setWithdrawAmount: 更新取款金额的函数
  ...
  // Approve transaction
  // approveHash: 交易哈希, 初始值为空字符串
  // isApproving: 交易是否正在发送到区块链, 初始值为 false
  // isApproveConfirming: 交易是否正在确认, 初始值为 false
  // isApproveSuccess: 交易是否成功, 初始值为 false
  ...
  // 当用户点击 Approve 按钮时，调用 approve 函数发送 Approve 交易
  const handleApprove = () => {
